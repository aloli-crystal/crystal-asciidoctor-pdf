...
describe AsciidoctorPDF::HyphCli do
  ...
  describe ".extract_lang_from_filename" do
    it "extrait la langue d'un nom de fichier hyph-*.tex" do
      AsciidoctorPDF::HyphCli.extract_lang_from_filename("hyph-fr.tex").should eq "fr"
      AsciidoctorPDF::HyphCli.extract_lang_from_filename("hyph-en-us.tex").should eq "en-us"
      AsciidoctorPDF::HyphCli.extract_lang_from_filename("hyph-zh-latn-pinyin.tex").should eq "zh-latn-pinyin"
    end

    it "retourne nil pour un nom de fichier qui ne suit pas le motif" do
      AsciidoctorPDF::HyphCli.extract_lang_from_filename("README.md").should be_nil
      AsciidoctorPDF::HyphCli.extract_lang_from_filename("hyph-fr.txt").should be_nil
      AsciidoctorPDF::HyphCli.extract_lang_from_filename("fr.tex").should be_nil
    end
  end
  ...
  describe ".parse_available_response" do
    it "parse une réponse JSON de l'API GitHub" do
      ...
      langs = AsciidoctorPDF::HyphCli.parse_available_response(body, url)
  ...
  describe ".available_cache_valid?" do
    it "retourne false quand le cache n'existe pas" do
      AsciidoctorPDF::HyphCli.available_cache_valid?.should be_false
    end

    it "retourne true quand le cache est récent" do
      Dir.mkdir_p(AsciidoctorPDF::HyphCli.xdg_cache_dir)
      File.write(AsciidoctorPDF::HyphCli.available_cache_file, %({"langs":[]}))
      AsciidoctorPDF::HyphCli.available_cache_valid?.should be_true
    end

    it "retourne false quand le cache est plus vieux que la TTL" do
      Dir.mkdir_p(AsciidoctorPDF::HyphCli.xdg_cache_dir)
      path = AsciidoctorPDF::HyphCli.available_cache_file
  ...
  describe ".read_cached_langs" do
    it "lit la liste de langues d'un cache bien formé" do
      Dir.mkdir_p(AsciidoctorPDF::HyphCli.xdg_cache_dir)
      File.write(
        AsciidoctorPDF::HyphCli.available_cache_file,
        {
      ...
      langs = AsciidoctorPDF::HyphCli.read_cached_langs
      langs.should eq ["fr", "en-us", "ja"]
    end

    it "retourne nil quand le cache est absent" do
      AsciidoctorPDF::HyphCli.read_cached_langs.should be_nil
    end

    it "retourne nil quand le cache est corrompu" do
      Dir.mkdir_p(AsciidoctorPDF::HyphCli.xdg_cache_dir)
      File.write(AsciidoctorPDF::HyphCli.available_cache_file, "{ not json")
      AsciidoctorPDF::HyphCli.read_cached_langs.should be_nil
    end
  end
  ...
  describe ".available_source_url" do
    it "préfère HYPH_MIRROR à CTAN_MIRROR à l'URL par défaut" do
      AsciidoctorPDF::HyphCli.available_source_url.should eq AsciidoctorPDF::HyphCli::DEFAULT_AVAILABLE_URL
      ENV["CTAN_MIRROR"] = "https://ctan.example.org/"
      AsciidoctorPDF::HyphCli.available_source_url.should eq "https://ctan.example.org/"
      ENV["HYPH_MIRROR"] = "https://hyph.example.org/"
      AsciidoctorPDF::HyphCli.available_source_url.should eq "https://hyph.example.org/"
    end
  end
  ...
  describe "cmd_available (intégration via binaire)" do
    ...
    it "rejette un flag inconnu avec exit 2" do
      ...
      status.exit_code.should eq AsciidoctorPDF::HyphCli::EXIT_USAGE
    end
